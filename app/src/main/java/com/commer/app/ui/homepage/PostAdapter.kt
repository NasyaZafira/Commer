...
class PostAdapter(
    var allResponse: MutableList<Data>,
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.binding.btnMore.setOnClickListener {
            if (response.user.id == getIdUser) {
                if (response.filePosts.isNotEmpty()) {
                    optionsMenuClickListener.onOptionsMenuForUser(
                        it,
                        response.idPost,
                        position,
                        response.postDesc,
                        response.user.fullname,
                        response.filePosts[0].url,
                        dataPost
                    )
                } else {
                    optionsMenuClickListener.onOptionsMenuForUser(
        ...
        holder.binding.icLove.setOnClickListener {
            optionsMenuClickListener.onLikeClicked(response.idPost, position)
        }
        holder.binding.icSolidLove.setImageResource(R.drawable.ic_solid_love)
        holder.binding.icSolidLove.setOnClickListener {
            optionsMenuClickListener.onUnlikeClicked(response.idPost, position)
        }
        ...
        holder.binding.icSavePost.setOnClickListener {
            optionsMenuClickListener.onBookmarkClicked(response.idPost, position)
        }
        ...
        holder.binding.icSolidSavePost.setOnClickListener {
            optionsMenuClickListener.onDeleteBookmarkClicked(response.idPost, position)
        }
