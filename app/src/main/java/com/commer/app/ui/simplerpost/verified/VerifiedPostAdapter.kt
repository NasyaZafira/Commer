...
class VerifiedPostAdapter(
    var verifiedPost: MutableList<Data>,
    private val onItemClick: (Data) -> Unit,
    private val onVideoClick: (Data) -> Unit,
    private val onProfileClick: (Data) -> Unit,
    private var onClickedAtVerifiedPost: OnClickedAtVerifiedPost
) : RecyclerView.Adapter<VerifiedPostAdapter.ViewHolder>() {

    interface OnClickedAtVerifiedPost {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val response = verifiedPost[position]
        ...
        holder.binding.icLove.setOnClickListener {
            onClickedAtVerifiedPost.onLikeClicked(response.idPost, position)
        }
        holder.binding.icSolidLove.setImageResource(R.drawable.ic_solid_love)
        holder.binding.icSolidLove.setOnClickListener {
            onClickedAtVerifiedPost.onUnlikeClicked(response.idPost, position)
        }
        ...
        holder.binding.icSavePost.setOnClickListener {
            onClickedAtVerifiedPost.onBookmarkClicked(response.idPost, position)
        }
        ...
        holder.binding.icSolidSavePost.setOnClickListener {
            onClickedAtVerifiedPost.onDeleteBookmarkClicked(response.idPost, position)
        }
        ...
        holder.binding.btnMore.setOnClickListener {
            if (response.user.id == getIdUser) {
                onClickedAtVerifiedPost.onOptionsMenuUserClicked(
                    it,
                    response.idPost,
                    position,
                    dataPost
                ...
                onClickedAtVerifiedPost.onOptionsMenuOtherUserClicked(
                    it,
                    response.idPost,
                )
            }
    ...
    override fun getItemCount() = verifiedPost.size
